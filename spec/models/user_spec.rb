...
# Microposts
  describe "micropost associations" do
    before :each do
      @user = User.create(@attr)
      @mp1 = Factory(:micropost, :user => @user, :created_at => 1.week.ago)
      @mp2 = Factory(:micropost, :user => @user, :created_at => 1.day.ago)
      @mp3 = Factory(:micropost, :user => @user, :created_at => 1.hour.ago)
    end
    ...
    it "should destroy associated microposts of a destroyed user" do
      @user.destroy
      [@mp1, @mp2, @mp3].each do |mp|
    ...
    describe "status feed" do
      ...
      it "should not include other user's microposts" do
        mpX = Factory(:micropost, :user => Factory(:user, :email => Factory.next(:email)))
        @user.feed.include?(mpX).should be_false
      end
    end
  end
end
